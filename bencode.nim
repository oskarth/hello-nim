...
proc decode(this: Decoder, source: string) : (BencodeType, int) =
    ...
    while idx < source.len:
        ...
        case curchar
        of 'i':
            let pair = this.decode_i(source[idx..source.len-1])
            let obj = pair[0]
            let nextobjpos = pair[1]
            idx += nextobjpos
            return (obj, idx)
        of 'l':
            let pair = this.decode_l(source[idx..source.len-1])
            let obj = pair[0]
            let nextobjpos = pair[1]
            idx += nextobjpos
            return (obj, idx)
        of 'd':
            let pair = this.decode_d(source[idx..source.len-1])
            let obj = pair[0]
            let nextobjpos = pair[1]
            idx += nextobjpos
            return (obj, idx)
        else:
            let pair = this.decode_s(source[idx..source.len-1])
            let obj = pair[0]
            let nextobjpos = pair[1]
            idx += nextobjpos
            return (obj, idx)
